...
std::vector<std::string> center(const std::vector<std::string> &vec)
{
	...
	{
		std::string::size_type padding = max_width - vec[i].size();
		if(padding % 2 == 0)
		{
			...
			temp.append(pad);
			vec[i] = temp;*/
			ret.push_back(std::string(padding/2,' ') + vec[i] + std::string(padding/2,' ' ));
		}
		else
		{
			ret.push_back(std::string(padding/2,' ') + vec[i] + std::string(padding/2 + 1,' ' ));
		}
	}
...
int main()
{
	...
	std::vector<std::string> result = center(test);
